...
- **SDUI package** — `packages/sdui/` (if new primitives were created)
...
2. Read `packages/tercen_ui_widgets/catalog.json`. Locate the new widget entry by `metadata.type`.
...
`packages/tercen_ui_widgets/catalog.json` must parse without errors. Use `python3 -c "import json; json.load(open('packages/tercen_ui_widgets/catalog.json'))"` or equivalent.
...
Run `cd packages/sdui && dart analyze`. No errors allowed. Warnings noted but not FAIL.
